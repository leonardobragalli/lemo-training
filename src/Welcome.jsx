...
const Welcome = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-[100dvh] flex flex-col md:flex-row font-sans overflow-y-auto md:overflow-hidden relative bg-[#03091B]">
      ...
      <div className="hidden md:flex md:w-[50%] lg:w-[55%] p-16 lg:p-24 flex-col justify-between relative z-10">
        
        <div className="relative z-10 mt-12">
          <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 200, damping: 20 }} className="mb-8 inline-block">
            <img src="/images/logo-character-photoroom.png" alt="Lemons in the room" className="h-56 object-contain drop-shadow-2xl" />
          </motion.div>
          <motion.h1 initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="text-6xl lg:text-[6rem] font-black font-serif text-[#03091B] mb-8 leading-[1.1] tracking-tight drop-shadow-2xl pr-10 overflow-visible antialiased" style={{ WebkitFontSmoothing: 'antialiased', MozOsxFontSmoothing: 'grayscale' }}>
            Il training <br/>
            <span className="relative inline-block mt-4 overflow-visible">
              <span className="relative z-10 text-transparent bg-clip-text bg-gradient-to-r from-[#FF8731] to-[#FF9E54] pr-4">che accoglie.</span>
            </span>
          </motion.h1>
          <motion.p initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3, duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="text-[#03091B]/70 text-2xl max-w-xl font-bold leading-relaxed">
            Preparati a vivere la realtà virtuale in reparto. Un percorso formativo semplice, intuitivo e sicuro.
          </motion.p>
        </div>
        
        {/* Placeholder div to keep flex-between spacing if needed, since security badge moved */}
        <div></div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-12 relative z-10">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="w-full max-w-[540px] relative z-10 bg-black/20 backdrop-blur-[40px] p-8 pt-4 md:p-14 rounded-[3.5rem] shadow-xl border-t border-l border-white/20 border-r border-b border-white/5 my-8 md:my-0 group/glass">
          ...
          <motion.div variants={itemVariants} className="mb-10 flex items-center justify-center gap-4 bg-white/10 backdrop-blur-3xl p-4 rounded-3xl border border-white/10 shadow-lg relative z-10 mx-auto w-fit">
            <div className="w-12 h-12 bg-gradient-to-br from-[#FF8731]/20 to-[#FF8731]/5 rounded-xl flex items-center justify-center border border-[#FF8731]/20 shadow-inner shrink-0">
              <ShieldCheck className="w-6 h-6 text-[#FF8731]" />
            </div>
            <div className="text-left">
              <span className="block text-white text-sm font-bold tracking-wide">Protocollo Sicurezza Ospedaliera</span>
              <span className="block text-slate-300 text-[10px] font-medium mt-0.5 flex items-center gap-1"><Sparkles className="w-3 h-3 text-[#FF8731]" /> Lemons in the Room Certified</span>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="mb-6 md:mb-8 text-center relative z-10">
            ...
            <p className="text-slate-300 font-medium text-lg leading-relaxed mt-4">Inserisci le tue credenziali operative per sbloccare un'esperienza di training immersiva.</p>
          </motion.div>

          <form onSubmit={handleSubmit} className="space-y-6 relative z-10 text-left">
            <div className="grid grid-cols-2 gap-5">
              <motion.div variants={itemVariants} className="space-y-2">
                <label className="text-[13px] font-bold text-white uppercase ml-2 tracking-widest drop-shadow-md">Nome</label>
                <div className="relative group">
                  <Type className="absolute left-6 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-white transition-colors duration-300 z-10" />
                  <input type="text" required className="block w-full pl-14 pr-6 py-5 bg-black/40 border border-white/10 focus:bg-[#8756FA]/10 rounded-[2rem] text-white focus:ring-2 focus:ring-[#8756FA]/50 focus:border-[#8756FA] transition-all duration-300 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)] focus:shadow-[0_0_25px_rgba(135,86,250,0.4),inset_0_2px_10px_rgba(0,0,0,0.5)] outline-none font-bold text-lg placeholder-slate-500" placeholder="Mario" value={firstName} onChange={(e) => setFirstName(capitalize(e.target.value))} />
                </div>
              </motion.div>
              <motion.div variants={itemVariants} className="space-y-2">
                <label className="text-[13px] font-bold text-white uppercase ml-2 tracking-widest drop-shadow-md">Cognome</label>
                <div className="relative group">
                  <SquareUser className="absolute left-6 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-white transition-colors duration-300 z-10" />
                  <input type="text" required className="block w-full pl-14 pr-6 py-5 bg-black/40 border border-white/10 focus:bg-[#8756FA]/10 rounded-[2rem] text-white focus:ring-2 focus:ring-[#8756FA]/50 focus:border-[#8756FA] transition-all duration-300 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)] focus:shadow-[0_0_25px_rgba(135,86,250,0.4),inset_0_2px_10px_rgba(0,0,0,0.5)] outline-none font-bold text-lg placeholder-slate-500" placeholder="Rossi" value={lastName} onChange={(e) => setLastName(capitalize(e.target.value))} />
                </div>
              </motion.div>
            </div>
            
            <motion.div variants={itemVariants} className="space-y-2">
              <label className="text-[13px] font-bold text-white uppercase ml-2 tracking-widest drop-shadow-md">Ospedale</label>
              <div className="relative group">
                <Building2 className="absolute left-6 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-white transition-colors duration-300 z-10" />
                <input type="text" required className="block w-full pl-14 pr-6 py-5 bg-black/40 border border-white/10 focus:bg-[#FF8731]/10 rounded-[2rem] text-white focus:ring-2 focus:ring-[#FF8731]/50 focus:border-[#FF8731] transition-all duration-300 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)] focus:shadow-[0_0_25px_rgba(255,135,49,0.3),inset_0_2px_10px_rgba(0,0,0,0.5)] outline-none font-bold text-lg placeholder-slate-500" placeholder="Es. Careggi" value={hospital} onChange={(e) => setHospital(capitalize(e.target.value))} />
              </div>
            </motion.div>
            
            <motion.div variants={itemVariants} className="space-y-2">
              <label className="text-[13px] font-bold text-white uppercase ml-2 tracking-widest drop-shadow-md">Reparto</label>
              <div className="relative group">
                <Stethoscope className="absolute left-6 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-white transition-colors duration-300 z-10" />
                <input type="text" required className="block w-full pl-14 pr-6 py-5 bg-black/40 border border-white/10 focus:bg-[#FF8731]/10 rounded-[2rem] text-white focus:ring-2 focus:ring-[#FF8731]/50 focus:border-[#FF8731] transition-all duration-300 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)] focus:shadow-[0_0_25px_rgba(255,135,49,0.3),inset_0_2px_10px_rgba(0,0,0,0.5)] outline-none font-bold text-lg placeholder-slate-500" placeholder="Es. Oncologia" value={department} onChange={(e) => setDepartment(capitalize(e.target.value))} />
              </div>
            </motion.div>
